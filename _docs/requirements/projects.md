# プロジェクト・セクション・プロジェクト所属

## 概要

本システムは、組織（organization）配下にプロジェクト（project）を置き、プロジェクト単位でタスクを管理する。  
プロジェクトにはセクション（section）で作業を分類でき、ユーザーは **組織所属（memberships）** を前提として **プロジェクト所属（project_memberships）** により参加する。

## 本書の範囲

- プロジェクト・セクション・プロジェクトメンバーに関する**要件定義**（満たすべき挙動）を記載する。
...
- Project: タスク・セクションが所属する作業単位（organization の子）
- Section: プロジェクト内でタスクを分類する単位（`docs/requirements/tasks.md` と同一）
...
- Archived project: 運用上ロックしたプロジェクト（`archived_at IS NOT NULL`）。**作成・削除・構造変更**（メンバー変更・セクション変更）を禁止する（詳細は「プロジェクトのアーカイブ・復帰・削除」）。
...
- プロジェクト作成画面
- プロジェクト編集画面
- セクション一覧・編集 UI（プロジェクト詳細に内包してもよい）
- プロジェクトメンバー管理画面
...
- プロジェクト作成時に**既定セクション**を自動作成するかは任意とする。作成する場合は `name` および `sort_order` の初期値は `docs/ui` / `docs/api` で定義する。
...
- **アーカイブ済みプロジェクト**（`archived_at IS NOT NULL` かつ `deleted_at IS NULL`）では、次を**すべて禁止**し、要求は拒否する。  
  - **作成**: 当該プロジェクト配下の**タスク新規作成**、**セクション新規作成**を含む（その他の新規リソース作成の有無は `docs/api` で補足してよい）。  
  - **削除**: **当該プロジェクトの論理削除**（`deleted_at` の設定）、**セクション削除**、**当該プロジェクト配下タスクの削除**（論理削除を含む）を含む。  
  - **構造変更**: **メンバー変更**（`project_memberships` の追加・削除・`role` 変更）、**セクション変更**（セクションの作成・更新・削除・並び替え）。  
...
- セクションは**プロジェクトに紐づく**。セクションの CRUD の権限は **`docs/permissions`（セクション管理権限）で定義**する。
...
- セクション削除時、**当該セクションに所属するタスク**については **`task.section_id` を `null` に設定**する（タスク自体は削除しない）。
- **アーカイブ済みプロジェクト**に対するセクション操作は、「プロジェクトのアーカイブ・復帰・削除」の**構造変更禁止**に従い**禁止**する。

### エラー・例外時の扱い

- 存在しないプロジェクト ID・セクション ID 指定時は要求を拒否する。
...
物理設計・インデックスは `docs/database` に記載する。`tasks` / `task_comments` / `task_histories` / `sections` の列定義は `docs/requirements/tasks.md` と重複するため、**以降はプロジェクト周辺の核のみ**示す。
...
- メンバー追加・削除、セクション並び替えは、可能な範囲で冪等に扱う。
- `due_date` や表示時刻と同様、**アーカイブ・削除の時刻判定**はサーバー時刻（UTC）で統一し、表示時のみローカルタイムゾーンへ変換する。

### 監査可能性

- 記録対象: プロジェクト作成、更新、アーカイブ、復帰、論理削除、メンバー追加・削除、セクション作成・更新・削除・並び替え
- 記録項目: 実行者 `user_id`、対象 `project_id` / `section_id`、組織 `organization_id`、変更要点、**結果**（`success` / `failure` を含む）、実行日時。**`failure` の場合**は、**理由を表すコード**（例: 権限不足・バリデーションエラー種別）を**含めてもよい**（コード体系は `docs/api` または運用方針で定義する）。
- **権限不足・バリデーションエラーなどにより拒否された操作**を監査ログに含めるかどうかは **`docs/permissions` または運用方針で別途定義**する。
- 監査ログに機微情報（秘密トークン等）を保存しない。

## 備考（運用ルール）

- プロジェクト名・セクション名の文字数上限・禁止文字は `docs/api` で定義する。
- 組織作成直後の**初期プロジェクト**の自動生成有無はプロダクト方針として任意とし、採用する場合は `docs/ui` / オンボーディング設計に明記する。
- `docs/requirements/tasks.md` のタスク・コメント・履歴の制約（`project_id` / `section_id` 整合、`project_memberships` 前提）は、本書の方針と**矛盾しないこと**。

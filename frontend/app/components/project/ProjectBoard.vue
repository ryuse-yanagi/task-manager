<template>
  <div
    class="project-board board-page"
            ...
            >
              <template #item="{ element: list }">
                <article
                  ...
                  :style="{ backgroundColor: listColumnBackgroundColor(list.color) }"
                >
              <header
                class="list-header"
...
import { listColumnBackgroundColor } from '../../constants/labelColorPresets'
...
type ListDef = { key: string; title: string; listId: number; color: string }
...
async function onListCreateSubmit ({ name, color }: { name: string; color: string }) {
  if (listCreateLoading.value) return
  listCreateLoading.value = true
  try {
    await api<{ id: number; name: string; color: string; sort_order: number }>(
      `/orgs/${slug.value}/projects/${projectId.value}/lists`,
      {
        method: 'POST',
        body: { name, color, sort_order: lists.value.length },
      },
    )
...
async function onListColumnDragEnd () {
  const hadPendingPreview = listColumnDragPreviewRaf !== 0

...
import { useProjectBoardPageData, boardTaskToTaskDetail } from '../../composables/useProjectBoardPageData'
...
import ProjectWbsGanttColorPopover from './ProjectWbsGanttColorPopover.vue'
...
const detailInitialTask = computed((): TaskDetail | null => {
  ...
  return enrichTaskDetailHierarchy(
    ...
    tasks.value,
    listId => projectLists.value.find(list => list.id === listId)?.name ?? null,
  )
})
...
const detailHierarchyTasks = computed(() => {
  return tasks.value.map(task => ({
    ...
    list_name: task.list_name ?? projectLists.value.find(list => list.id === task.list_id)?.name ?? null,
...
function persistWbsCache () {
  ...
  setWbsCached(props.orgSlug, props.projectId, {
    tasks: tasks.value,
...
async function saveGanttBarColor (color: string) {
  ...
  try {
    await api(
      `/orgs/${props.orgSlug}/projects/${props.projectId}/tasks/${taskId}`,

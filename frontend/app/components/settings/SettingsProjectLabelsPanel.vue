<template>
  <div class="settings-label-panel-content">
    ...
    <ul class="settings-label-list">
      ...
      <li v-if="!labels.length" class="settings-label-empty">まだプロジェクトラベルはありません。</li>
    </ul>
...
async function load () {
  loading.value = true
  try {
    const res = await api<{ data: SettingsLabelItem[] }>(`/orgs/${props.orgSlug}/project-labels`)
...
async function createLabel (payload: { name: string; color: string }) {
  ...
  setMessage('', 'ok')
  try {
    const created = await api<SettingsLabelItem>(`/orgs/${props.orgSlug}/project-labels`, {
      method: 'POST',
      body: {
        name,
        color: payload.color,
    ...
    setMessage('プロジェクトラベルを作成しました。', 'ok')
  } catch (e: unknown) {

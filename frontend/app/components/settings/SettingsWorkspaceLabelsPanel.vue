<template>
  <div class="settings-label-panel-content">
    ...
    <ul class="settings-label-list">
      ...
      <li v-if="!labels.length" class="settings-label-empty">まだワークスペースラベルはありません。</li>
    </ul>
    <LabelCreateModal
      v-model="modalOpen"
      title="ワークスペースラベルの作成"
...
async function load () {
  loading.value = true
  try {
    const res = await api<{ data: SettingsLabelItem[] }>(`/orgs/${props.orgSlug}/workspace-labels`)
    labels.value = resolveLabelColors(res.data)
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : 'ワークスペースラベル取得に失敗しました'
    setMessage(msg, 'err')
  } finally {
    loading.value = false
  }
...
async function createLabel (payload: { name: string; color_index: number }) {
  ...
  setMessage('', 'ok')
  try {
    const created = await api<SettingsLabelItem>(`/orgs/${props.orgSlug}/workspace-labels`, {
      method: 'POST',
      body: {
        name,
    ...
    setMessage('ワークスペースラベルを作成しました。', 'ok')
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : 'ワークスペースラベル作成に失敗しました'
    setMessage(msg, 'err')
  } finally {
    loading.value = false
  }
